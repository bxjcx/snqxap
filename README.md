...
###v.2.1.1 (末日时在做什么?有没有空?可以来少女前线吗?版) 2017.08.07

 > - 增加可以拉伸的边框，使对缩放友好一些。
 > - 又加了俩按钮最小化和最大化，暂时不知道放哪合适先扔那。
 > - 可能修正了移动窗口和slider的冲突。
 > - 修复妖精星级更换时重置buff加成。
 > - 增加空降技能效果。
 > - 修改防御妖精技能实现，在计算甲之后减伤。
...
###v2.1.0 (历史总是惊人的相似版) 2017.06.29

 > - 更新仓促，工作量大，UI太菜，本次更新可能有一万个bug，欢迎反馈。
 > - 下方三个按钮左边可以隐藏妖精栏、右边隐藏装备栏、中间关闭计算器。
 > - 右下角有一个箭头可以调整计算器大小。
 > - pkp技能额外发动的子弹可再次暴击。
 > - 修正按时间叠加类技能加成算法，现在为乘方。（内格夫史诗级加强）
 > - 修正G43技能初始CD。
 > - 修正L85A1光环指向。
 > - 修正护甲光环结算取整方式。
 > - 经验计算器增加夜战6图数据。
 > - 增加新枪数据。
 > - 增加倍卡选项。目前倍卡对404小队有效。
 > - 增加妖精相关功能。第一格为妖精、第二格分别为等级、星级、技能等级、技能发动。第三格为天赋。选择上天赋表示发动天赋。鼠标移到天赋和技能发动栏可以看到效果。
 > - 增加新装备数据。
 > - 夏活相关增加本次夏活主要铁血的数据。
